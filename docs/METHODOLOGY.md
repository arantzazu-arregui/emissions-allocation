...
| | Vessel A | Vessel B |
|---|---|---|
| Ship | COSCO ITALY, IMO 9516454 | ⚠ to be selected — §0 |
| Type | Container, 156,610 DWT, built 2014 | bulk carrier or tanker, mid-range DWT |
| Flag | Hong Kong | open registry (PAN / LBR / MHL / MLT / BHS / CYP) |
| Owner | Shanghai → China | a different country (GRC / JPN / DEU / …) |
...
|---|---|---|
| §1, §3, §4 | IMO notation | *Fourth IMO GHG Study 2020* |
| §2 | Naval-architecture convention | Charchalis (2014), Cepowski & Chorab (2021), MEPC.333(76) |
| §5–§7 | Allocation notation | Selin et al. (2021) |
...
|---|---|---|
| `i` | hourly index | – |
| `SOG_i` | speed over ground in hour *i* (derived, §1.5) | knots |
| `SOḠ_i` | smoothed speed over ground (§1.6) | knots |
...
| 3 | **Distinctive ship name** | the presence filter matches on name, exactly and case-sensitively (§1.2); a common name pulls multiple hulls |
...
1. Query 4Wings presence at world extent for one representative day, filtered `flag in ("PAN","LBR","MHL","MLT") AND vessel_type in ("cargo")`, grouped by `VESSEL_ID`. A single flag-and-type combination returned 991 distinct vessels for one day in testing, so this yields a large candidate pool cheaply.
...
### ⚠ Status
Vessel A is fixed. Vessel B has not been selected; steps 1–6 have not been run.
...
`SOG` is the IMO's term and the AIS field name, but note the difference in provenance: in the Fourth GHG Study `SOG` is *transmitted* by the vessel, whereas here it is *derived* from consecutive cell centroids. §1.6 exists because of that difference.
...
⚠ *w* is a free parameter, currently validated on one day of data. It is one of the two variables carried through the sensitivity analysis (§8).
...
### Limitations
Positions are cell centroids, not true fixes. Speed is derived, not transmitted — GFW does not expose AIS speed over ground directly. AIS navigational status is unavailable, which matters in §4. Presence hours are observed-only.

---

# 2. Obtain ship specifications and parameters
...
| TEU capacity | derived, §2.1 | ≈ 13,200 | **estimated** |
| Design speed `V` | derived, §2.2 | three estimates | **estimated** |
| Installed power `MCR` | derived, §2.2 | three estimates | **estimated** |
| Engine type | assigned, §2.3 | slow-speed diesel | assigned |
| Owner / manager / operator country | Equasis | see §5 | observed |
...
# 3. Define fuel type and emission factors
...
### Inputs
`vessel_hour` (§1), `vessel_specs` (§2), `fuel_assignment` (§3), IMO Tables 16 and 17.
...
with `Ẇ_ME,i = 0` where `Load_i < 0.07` (§3.5) and in At berth / Anchored modes.
...
`LLF` does not appear because CO₂'s low-load factor is 1.00 at every load (§3.5).
...
"Scenario" is the cross join of three power/speed estimates × the smoothing-window values (§8).
...
### Inputs
`emissions_year` (§4); Equasis company records; EEZ v12 and High Seas v2 for the domestic/international test.

### 5.1 Allocation keys

| Option | Source | Vessel A (COSCO ITALY) | Vessel B |
|---|---|---|---|
| Flag country | Equasis / GFW registry | **Hong Kong** | open registry — PAN / LBR / MHL / MLT |
| Owner country | Equasis registered owner | COSCO ITALY SHIPPING LTD, Shanghai → **China** | ⚠ different from flag, by construction |
| Manager country | Equasis ISM manager | SHANGHAI OCEAN SHIPPING CO LTD → **China** | ⚠ to be determined |
| Operator country | Equasis commercial manager (proxy) | COSCO SHIPPING LINES CO LTD → **China** | ⚠ to be determined |
...
1. **Installed power and design speed** — three estimates (§2.2), carried in parallel with no primary.
...
| # | Item | Blocks |
|---|---|---|
| 1 | **Vessel B not yet selected** — run §0.2, then Equasis for its four allocation keys and DWT | §0, §2, §5, §7 |
| 2 | Selin et al. supplementary Table 1 (territory merging, Hong Kong treatment) | §5.4, §6.3 |
| 3 | Coastline layer for distance-to-coast | §4.1 |
| 4 | Sourced installed power and service speed per hull | §2.2 estimate C |
| 5 | Smoothing window validated on more than one day | §1.6, §8.1 |
| 6 | Joint-regime / overlapping-claim EEZ rule | §5.4 |
