...
assigns an operating mode from *main-engine load*, while Section 5 zeroes main-engine
...
# Section 4: no main-engine power at berth or at anchor.
...
def main_engine_load(
    ...
    cube is why Section 1.2's smoothing is mandatory rather than cosmetic: cell-centroid
...
def build_hour_model(
    ...
    # Section 4: main engine off at berth and at anchor, and below the 7% MCR cutoff.
    ...
    # Fuel is assigned per hour in Section 5; SFC and EF are looked up from it in SQL via
...
def register_distance_layers(db: Database, cfg: Config, spine: pd.DataFrame) -> pd.DataFrame:
    ...
    # Section 4 at-berth/anchored: a port-visit interval is a stronger signal than
